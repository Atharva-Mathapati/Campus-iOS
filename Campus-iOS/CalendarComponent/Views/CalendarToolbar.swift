            ...
            NavigationLink(destination:
                VStack{
                    GeometryReader { geo in
                        CalendarDisplayView(events: viewModel.events.map({ $0.kvkEvent }), type: .list, selectedEventID: self.$selectedEventID, frame: CalendarContentView.getSafeAreaFrame(geometry: geo), todayPressed: self.$isTodayPressed, isWidget: false)
